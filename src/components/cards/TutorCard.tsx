...
export function TutorCard({ tutor }: { tutor: Tutor }) {
  const img = (tutor as { image?: string }).image || tutorImage(tutor.id);

  return (
    <Link to="/tutors/$id" params={{ id: tutor.id }} className="group block h-full">
      <article className="flex h-full flex-col rounded-2xl border bg-card transition-shadow hover:shadow-md">
        <div
