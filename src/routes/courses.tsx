...
function FP({ cat, setCat, level, setLevel, price, setPrice, rating, setRating, cert, setCert }: any) {
  return (
    <div className="space-y-6">
      ...
      <div>
        <Label className="text-xs uppercase tracking-wide text-muted-foreground">Max price · ${price[0]}</Label>
...
function CoursesPage() {
  ...
  const list = useMemo(
    ...
    [q, cat, level, price, rating, cert],
  );

  const fp = { cat, setCat, level, setLevel, price, setPrice, rating, setRating, cert, setCert };
    ...
    <section className="container mx-auto px-4 py-10">
      ...
      <div className="grid lg:grid-cols-[260px_1fr] gap-8">
        ...
        <div>
          ...
          {list.length === 0 && <div className="text-center py-20 text-muted-foreground">No courses match your filters.</div>}
        </div>
      </div>
    </section>
  );
